...
import com.ams.resource.PasswordEncryptor;
import com.ams.resource.PasswordDecryptor;
...
public class RestResourse {
	...
	public @ResponseBody ResponseEntity<String> getAdminPassword(@PathVariable String id) {
		...
		if (!list.isEmpty()) {
			return new ResponseEntity<String>(list.get(0).getPassword(), HttpStatus.OK);
		}
	...
	public @ResponseBody ResponseEntity<String> getCustomerPassword(@PathVariable String id) {
		...
		if (!list.isEmpty()) {
			return new ResponseEntity<String>(PasswordDecryptor.passwordDecrypt(list.get(0).getPassword()), HttpStatus.OK);
		}
		return new ResponseEntity<String>(HttpStatus.NOT_FOUND);
	}
	...
	public ResponseEntity<String> registerCustomer(@RequestBody CustomerHolder ch) {
		...
		cp.setPassword(PasswordEncryptor.passwordEncrypt(ch.getPassword()));
